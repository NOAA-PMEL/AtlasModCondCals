...
class ModuleMeta(object):
   ...
   """
    ...
    def meta_summary(self, compclock, tolerance, prefix=0):
      ...
      """
        dtdelta = int(compclock[0] - self.dumpdt)
        ...
        if compclock[1]:
            logger.info(
                "NTP Time           : %s"
                % (time.strftime(isofmt, time.localtime(compclock[0])),)
            )
        else:
            logger.info(
                "Computer Time      : %s"
        ...
        if self.badclock:
            logger.warning(
                "Module Clock Error : %s %d d %02d:%02d:%02d"
...
def check_ntp_server(ntpserv):
    ...
    try:
        ...
        sys.stderr.write(
            "*** localhost system clock is %.2f seconds behind time server %s ***\n\n"
            % (response.delay, ntpserv)
        )
    except:
        sys.stderr.write(
            "*** time server %s not found, using localhost system clock ***"
        )
    sys.stderr.flush()
...
def dump_data(ser, meta, args):
   ...
   """
    ...
    logger.info(
        "Dumped %s records in %.1f seconds" % (meta.ndumprec, (dumpend - dumpst))
    )
...
check_ntp_server(args.ntpserver)
    ...
    while 1:
        ...
        #          continue
            ...
            if select.upper() == "S":
                ...
                tmcheck, header = wake_ssc_get_header(ser, args.ntpserver, args.debug)
            # dump TC: control-C to start
            elif select.upper() == "T":
                tmcheck, header = wake_tc_get_header(ser, args.ntpserver, args.debug)
            ...
            if header and header.strip():
                ...
                if modmeta.parseheader():
                    ...
                    logger.warning(
                        "Could not parse header: %s" % repr(modmeta.rawheader)
                    )
                    if ser.in_waiting:
                        clear_input_buffer(ser)
                    continue
